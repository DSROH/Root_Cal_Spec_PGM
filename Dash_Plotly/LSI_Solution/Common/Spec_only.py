...
import LSI_Solution.DASEUL.Root.RX_2g as root_2grx
import numpy as np
...
def change_rx_spec(line, RX_Gain_Spec, text_area):
    ...
    if String[1] == "RsrpOffset":
        text_area.insert(
            tk.END,
            f"{String[0]:^4} RSRP   {String[2]:<6} {String[3]:<1}          | {String[5]:>5}\t{String[6]:>5}\t  \u2192  ",
        )
    else:
        text_area.insert(
            tk.END,
...
def Chng_2G_rx_gain_spec_only(Selected_spc, rat, band, RX_Gain_2G_Spec_var, text_area):
    ...
    for index, line in enumerate(data_lines):
        ...
        # Gainstate를 for로 할 경우 너무 많은 반복 -> if 문으로 처리
            ...
            String = re.split("\t|\n", line)
            String = [v for v in String if v]
            New_String = root_2grx.rx_gain(PRX_Gain_2G, RX_Gain_2G_Spec, band, "Spec_Only", String, "None", text_area)
            Change_Str = line.replace(Old_String, New_String)
            new_text_content += Change_Str
...
def apt_spec_only(line, APT_Spec):
    ...
    if (New_String[1] == "-10") & (New_String[2] == "-10.1") & (New_String[3] == "-9.9"):
        ...
        New_String[1] = math.floor(float(New_String[1]) + 0.5)
        New_String[2] = New_String[1] - float(APT_Spec)
        New_String[3] = New_String[1] + float(APT_Spec)
...
def Chng_rx_gain_spec_only_Slm(Selected_spc, rat, band, RX_Gain_Spec_var, text_area):
    RX_Gain_Spec = float(RX_Gain_Spec_var.get())
    new_text_content = ""
    Check = False
    ...
    for index, line in enumerate(data_lines):
        ...
        if line.startswith(Search_WD):
            ...
            text_area.insert(tk.END, f"\n\n{Search_WD}\n\n")
            # Gainstate를 for로 할 경우 너무 많은 반복 -> if 문으로 처리
        elif Check and line.startswith("RX_GAIN_Port"):
            New_String = change_rx_spec_Slm(line, RX_Gain_Spec, text_area)
            Change_Str = line.replace(Old_String, New_String)
            new_text_content += Change_Str
        elif Check and line.startswith("RX_RSSI_Offset_P"):
            New_String = change_rx_ch_spec_Slm(line, RX_Gain_Spec, text_area)
            Change_Str = line.replace(Old_String, New_String)
            new_text_content += Change_Str
        elif Check and line.startswith("RX_FREQ_Offset_P"):
            new_text_content += line
            Check = False
        else:
            new_text_content += line
...
def change_rx_spec_Slm(line, RX_Gain_Spec, text_area):
    String = [v for v in re.split("[=,\t\n]", line) if v]
    text_area.insert(
        tk.END,
        f"{String[0]:<30} | {String[2]:>5}\t{String[3]:>5}\t  \u2192  ",
    )
    RX_Gain_Value = [float(String[2]), float(String[3])]
    RX_Gain_mean = np.mean(RX_Gain_Value)

    String[1] = round(RX_Gain_mean)
    String[2] = round(RX_Gain_mean - RX_Gain_Spec)
    String[3] = round(RX_Gain_mean + RX_Gain_Spec)
...
def change_rx_ch_spec_Slm(line, RX_ch_Spec, text_area):
    String = [v for v in re.split("[=,\t\n]", line) if v]
    text_area.insert(
        tk.END,
        f"{String[0]:<30} | {String[2]:>5}\t{String[3]:>5}\t  \u2192  ",
    ...
    String[1] = round(RX_Gain_mean)
    String[2] = round(RX_Gain_mean - RX_ch_Spec)
    String[3] = round(RX_Gain_mean + RX_ch_Spec)
...
def Chng_fbrx_meas_spec_only_Slm(Selected_spc, band, FBRX_Meas_var, text_area):
    new_text_content = ""
    Check = False

    target_word = f"[SUB6_n{band}_Calibration_Spec]"
    FBRX_Spec = float(FBRX_Meas_var.get())
    End_word = f"TX_Main_TEMP_ADC"
    ...
    for index, line in enumerate(data_lines):
        ...
        if line.startswith(target_word):
            ...
            Check = True
        # Gainstate를 for로 할 경우 너무 많은 반복 -> if 문으로 처리
        elif Check and ("_FBRXGAIN_Pow_Index_" in line):
            New_String = fbrx_spec_Slm(line, FBRX_Spec, text_area)
            Change_Str = line.replace(Old_String, New_String)
            new_text_content += Change_Str
        elif Check and ("_FBRXGAIN_Code_Index_") in line:
            New_String = fbrx_spec_Slm(line, FBRX_Spec * 100, text_area)
            Change_Str = line.replace(Old_String, New_String)
            new_text_content += Change_Str
        elif Check and ("_FBRXFREQ_HPM_Pow") in line:
            New_String = fbrxfreq_spec_Slm(line, FBRX_Spec, text_area)
            Change_Str = line.replace(Old_String, New_String)
            new_text_content += Change_Str
        elif Check and ("_FBRXFREQ_LPM_Pow") in line:
            New_String = fbrxfreq_spec_Slm(line, FBRX_Spec, text_area)
            Change_Str = line.replace(Old_String, New_String)
            new_text_content += Change_Str
        elif Check and ("_FBRXFREQ_HPM_Code") in line:
            New_String = fbrxfreq_spec_Slm(line, FBRX_Spec * 100, text_area)
            Change_Str = line.replace(Old_String, New_String)
            new_text_content += Change_Str
        elif Check and ("_FBRXFREQ_LPM_Code") in line:
            New_String = fbrxfreq_spec_Slm(line, FBRX_Spec * 100, text_area)
            Change_Str = line.replace(Old_String, New_String)
            new_text_content += Change_Str
        elif Check and line.startswith(End_word):
...
def fbrx_spec_Slm(line, FBRX_Spec, text_area):
    String = [v for v in re.split("[=,\t\n]", line) if v]
    ...
    FBRX_Value = [float(String[2]), float(String[3])]
    FBRX_mean = np.mean(FBRX_Value)
    String[1] = round(FBRX_mean)
    String[2] = round(String[1] - FBRX_Spec)
    String[3] = round(String[1] + FBRX_Spec)
...
def fbrxfreq_spec_Slm(line, FBRX_Spec, text_area):
    String = [v for v in re.split("[=,\t\n]", line) if v]
...
def Chng_2G_rx_gain_spec_only_Slm(Selected_spc, rat, band, RX_Gain_2G_Spec_var, text_area):
    RX_Gain_2G_Spec = float(RX_Gain_2G_Spec_var.get())
    target_word = f"[{band}_Calibration_Spec]"
    new_text_content = ""
    Check = False
...
def tx_power_Slm(Spec_var, band, option, line, text_area):
    String = line.strip().split("\t")
    if option == "Power_TxL":
        text_area.insert(tk.END, f"{String[0]:<30}| {String[2]:>5}\t{String[3]:>5}\t\t\u2192\t")
        TxLevel = int(re.split("TxL|=|\t|\n", line)[1])
