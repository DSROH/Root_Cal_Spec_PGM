...
def Txp_channel_comp(line, band, TxP_Channel_comp_Spec_var, TxP_Channel_comp_ave):
    ...
    if Read_index < Index_count:
        New_String[1] = round(TxP_Channel_comp_ave[band].iloc[Read_index])
        New_String[2] = round(TxP_Channel_comp_ave[band].iloc[Read_index]) - TxP_Channel_comp_Spec_var
        New_String[3] = round(TxP_Channel_comp_ave[band].iloc[Read_index]) + TxP_Channel_comp_Spec_var
    else:
        New_String[1] = 0
...
def TxP_3g_channel_comp_pa_mid(df_Meas, Save_data, text_area):
